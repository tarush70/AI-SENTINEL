...
export default function Features() {
  ...
  const [huntStatus, setHuntStatus] = useState('idle'); // idle, scanning, analyzing, resolved
  const [patchStatus, setPatchStatus] = useState('idle'); // idle, intercepting, sandboxing, patched
  ...
  // Tab 2: Predictive Patching State (Deep AI-AppSec)
    ...
    <section className="relative bg-slate-950 text-slate-100 py-24 px-6 border-t border-slate-900">
      <div className="max-w-7xl mx-auto w-full">
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-12">
          ...
          <button 
            className="px-5 py-2.5 rounded-lg text-sm font-mono opacity-40 cursor-not-allowed border bg-slate-900/50 border-slate-800 text-slate-600"
            disabled
          >
            03 // Zero-Downtime Incident Response (Locked)
          </button>
        </div>

        {/* TAB 1 CONTENT: AUTONOMOUS THREAT HUNTING */}
        {activeTab === 'hunting' && (
        ...
        {/* TAB 2 CONTENT: PREDICTIVE AGENT PATCHING */}
        {activeTab === 'patching' && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            <div className="lg:col-span-5 space-y-6 bg-slate-900/40 border border-slate-800 p-6 rounded-2xl backdrop-blur-md">
              <h3 className="text-xl font-bold text-white flex items-center gap-2">
            ...
            <div className="lg:col-span-7 space-y-6">
              {/* Real-time Agent Diagnostic Panel */}
